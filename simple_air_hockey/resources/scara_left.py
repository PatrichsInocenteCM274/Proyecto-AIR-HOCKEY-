...
class ScaraL:
    ...
    def __init__(self, client,max_velocity = 7.0):
        self.client = client
        f_name = os.path.join(os.path.dirname(__file__), 'scara-left.urdf')
        self.scara_left = p.loadURDF(fileName=f_name,
        ...
        self.servos_joints = helper_id.get_id_joints_revolution(self.scara_left)
    ...
    def get_observation(self):
        ...
        pos = pos[:2]
        current_angle_servo = p.getJointState(self.scara_left, self.servos_joints[0])[0]
        current_angle_efector = p.getJointState(self.scara_left, self.servos_joints[1])[0]
